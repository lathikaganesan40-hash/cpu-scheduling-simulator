...
import copy
import random # Needed for random colors
...
def update_button_colors():
    """Changes buttons to a random color."""
    new_color = get_random_color()
    run_btn.config(bg=new_color)
...
def create_fields():
    try:
        ...
        messagebox.showerror("Input Error", f"Invalid input '{num_procs_entry.get()}'. Enter a positive integer.")
        return

    update_button_colors() # Change color when clicked
...
def draw_gantt(gantt):
    ...
    # We use a random colormap for the Gantt chart blocks too!
    cmap_names = ['Set3', 'Paired', 'Accent', 'Pastel1', 'Pastel2']
    colors = plt.cm.get_cmap(random.choice(cmap_names), 10)
...
def run_simulation():
    update_button_colors() # Change button color on run
    try:
        q_val = 1
        ...
        for i in range(len(arrival_entries)):
            ...
            if at < 0 or bt <= 0 or pr < 0: raise ValueError("Invalid process values.")
        ...
        messagebox.showerror("Error", f"Please check your inputs: {e}")
...
tk.Label(scroll_content, text="CPU SCHEDULING ALGORITHM SIMULATOR", font=("Helvetica", 18, "bold"), bg="#f0f4f8").pack(pady=15)
...
algo_menu = ttk.Combobox(left_panel, textvariable=algo_var, values=["FCFS", "SJF (Non-Preemptive)", "SJF (Preemptive)", "Priority (Non-Preemptive)", "Priority (Preemptive)", "Round Robin"], state="readonly")
...
run_btn.pack(fill="x", side="bottom", pady=10)
...
create_fields() # Initial creation
update_button_colors() # Initial random colors
root.mainloop()
