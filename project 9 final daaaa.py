...
def create_fields():
    ...
    for col,text in enumerate(headers):
        tk.Label(input_frame,text=text,bg="#1e3d59",fg="white",font=("Arial",10,"bold")).grid(row=0,column=col,padx=2,pady=2)

    for i in range(num_procs.get()):
        tk.Label(input_frame,text=f"P{i+1}",bg="#cce5ff").grid(row=i+1,column=0)
...
def run_simulation():
    ...
    for idx,p in enumerate(result):
        ...
        result_table.insert("", "end", values=(f"P{p.pid}",p.at,p.bt,p.pr,p.ct,p.tat,p.wt),tags=(tag,))
        avg_wt+=p.wt
        avg_tat+=p.tat
    avg_wt/=len(result)
    avg_tat/=len(result)
...
tk.Button(left_panel,text="Create Fields",bg="#00bfff",fg="white",font=("Arial",11,"bold"),
