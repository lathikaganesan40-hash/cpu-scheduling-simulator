...
def create_fields():
    ...
    for i in range(num_procs.get()):
        ...
        # Default values to prevent empty field crashes
...
def draw_gantt(gantt):
    ...
    # Filter out 0-duration blocks to prevent drawing errors
    ...
    for pid, start, duration in gantt:
        ...
        ax.text(start + duration/2, 0, f"P{pid}", ha='center', va='center', fontweight='bold')
...
def run_simulation():
    try:
        ...
        for i in range(num_procs.get()):
            # Input validation
            a_val = arrival_entries[i].get()
            b_val = burst_entries[i].get()
            p_val = priority_entries[i].get()
            
            if not (a_val and b_val and p_val):
                raise ValueError("All fields must be filled.")
                
            processes.append(Process(i+1, int(a_val), int(b_val), int(p_val)))
        
        # Check for 0 burst time which can break some logic
        if any(p.bt <= 0 for p in processes):
             messagebox.showwarning("Warning", "Burst time should be greater than 0 for accurate results.")
        ...
        avg_wt /= len(result)
        avg_tat /= len(result)
        avg_label.config(text=f"Average WT: {avg_wt:.2f} | Average TAT: {avg_tat:.2f}", fg="#2c3e50")
...
tk.Label(gantt_container, text="Gantt Chart", font=("Arial", 12, "bold"), bg="#f0f4f8", fg="#2c3e50").pack()
